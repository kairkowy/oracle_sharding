...
organization external(
type oracle_loader
default directory shard_dir
access parameters(
    records delimited by newline
    fields terminated by ',' (
    id, account_number, customer_id, branch_id, state, status
    )
    )location ('test_data1.csv')
)
reject limit unlimited
;


external table, itas 테스트 중
...
insert /*+ append parallel(2) */ into accounts
(select * from t1 where shard_chunk_id('SDTEST.ACCOUNTS', STATE) is not null);
...
(select * from sdtest.t1);

spool off

sqlldr sdtest/Welcome1 control=load.ctl

load data
infile './test_data1.csv'
into table t1
fields terminated by WHITESPACE optionally enclosed by '"'
(id, account_number, customer_id, branch_id, state, status)

insert into accounts (select * from t1);

insert /*+ append parallel(2) */ into accounts
(select * from t1 );

select chunk_number from gsmadmin_internal.chunks ;
where

insert into accounts values(255,2762,5572,22,'GG','Y');
insert into accounts values(622,1470,7878,57,'JJ','Y');

[](https://docs.oracle.com/en/database/oracle/oracle-database/19/shard/using-oracle-data-pump-migrate-sharded-database.html#GUID-87E61A9D-F57A-4A7B-A5CC-96FFD48A0C53)
...
select PHONE_NUMBERS. from customers;


리전

네트워크 대기 시간이 짧은 LAN 또는  MAN 상에서 네트워크 근접성을 공유하는 오라클 데이터베이스 및 GDS 구성의 부분집합임.
